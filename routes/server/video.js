...
const { noExt, dirOnly, removeThumbnails, isNewDate, formatDate } = require('../../helper')
const { generateDate, generateTitle, generateSite } = require('../../generate')
const { getWebsiteID, getSiteID } = require('../../helper.db')
...
router.post(
	'/',
	handler(async db => {
		const filesDB = await db.query('SELECT * FROM videos')
