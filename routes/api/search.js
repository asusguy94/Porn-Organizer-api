...
const { formatDate, getResizedThumb } = require('../../helper')
...
router.get(
	...
	handler(async db => {
		...
		const stars = []
		for (let i = 0, len = result.length, websiteArr = [], siteArr = [], star = {}; i < len; i++) {
			let prev = result[i - 1]
			let current = result[i]
			let next = result[i + 1]
